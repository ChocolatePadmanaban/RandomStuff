...
def average_perceptron(feature_matrix, labels, T):
    ...
    # Your code here
    theta, theta_0 = np.zeros(len(feature_matrix[0])),0
    sumtheta, sumtheta_0 = np.zeros(len(feature_matrix[0])),0
    for t in range(T):
        
        for i in get_order(feature_matrix.shape[0]):
            theta1 = perceptron_single_step_update(feature_matrix[i],labels[i], theta,theta_0)
            theta, theta_0 = theta1[0],theta1[1] 
            sumtheta, sumtheta_0 = sumtheta+ theta1[0],sumtheta_0+theta1[1]                     
            pass
    return (sumtheta/T/feature_matrix.shape[0],sumtheta_0/T/feature_matrix.shape[0])
    raise NotImplementedError
