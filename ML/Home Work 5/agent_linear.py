...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    MaxQ= np.max(theta  @ next_state_vector) if not terminal else 0
    theta[index] = theta[index] + ALPHA*(
        reward+ GAMMA*MaxQ -(theta @ current_state_vector)[tuple2index(
            action_index, object_index)])*current_state_vector 
